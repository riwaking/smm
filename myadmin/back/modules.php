...
function paymentStatu($statu) {
    ...
    elseif( $_GET["search_type"] == "payment_id" AND $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      ...
      $search_link  = "?search=".$search_word."&search_type=".$search_where;
    elseif( $_GET["search_type"] == "payment_amount" AND $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $count        = $conn->prepare("SELECT * FROM payments INNER JOIN clients ON clients.client_id = payments.client_id WHERE payments.payment_amount LIKE '%".$search_word."%' AND orders.dripfeed='1' AND orders.subscriptions_type='1' ");
      $count        -> execute(array());
      $count        = $count->rowCount();
...
elseif (route(2) == "online"):
    if ($_GET["search_type"] == "username" AND $_GET["search"]):
        ...
        $id.= ")";
        $search = " payments.client_id IN " . $id;
        $count = $conn->prepare("SELECT * FROM payments INNER JOIN clients ON clients.client_id = payments.client_id WHERE {$search} AND payments.payment_method!='6' AND payments.payment_status='3' ");
        $count->execute(array());
        $count = $count->rowCount();
        $search = "WHERE {$search} AND payments.payment_method!='6' AND payments.payment_status='3' ";
        $search_link = "?search=" . $search_word . "&search_type=" . $search_where;
    else:
        ...
        $count = $count->rowCount();
        $search = "WHERE payments.payment_method!='6' AND payments.payment_status='3' ";
    endif;
    $to = 50;
    $pageCount = ceil($count / $to);
    if ($page > $pageCount):
