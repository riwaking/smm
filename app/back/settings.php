...
$title .= $languageArray["account.title"] ?? "Settings";
...
$conn->beginTransaction();
  ...
  if( !userdata_check('password',md5($pass) ) ){
    $error    = 1;
    $errorText= $languageArray["error.account.password.notmach"] ?? "Current password doesn't match";
  }elseif( strlen($new_pass) < 8 ){
    $error    = 1;
    $errorText= $languageArray["error.account.password.length"] ?? "Password must be at least 8 characters";
  }elseif( $new_pass != $new_again ){
    $error    = 1;
    $errorText= $languageArray["error.account.passwords.notmach"] ?? "Passwords do not match";
  }else{
    $conn->beginTransaction();
      $insert= $conn->prepare("INSERT INTO client_report (client_id, action, report_ip, report_date) VALUES (:c_id, :action, :ip, :date)");
      ...
      $update = $update->execute(array("id"=>$user["client_id"],"pass"=>md5($new_pass) ));
        if( $update && $insert ):
          ...
          $conn->commit();
          $success    = 1;
          $successText= $languageArray["error.account.password.success"] ?? "Password changed successfully";

        else:
          $conn->rollBack();
          $error    = 1;
          $errorText= $languageArray["error.account.password.fail"] ?? "Password change failed";
